...
ad_val_mod = run_adversial_validation(train_copy, test_copy)# so  hospital_id, encounter_id are in the top, so we need to remove them
...
test[target_col]=0 #notice for concatenating!
...
train_groupby=df.groupby("gender") #fill missing data, according to visualization part
...
df.loc[df.age == 0, 'age'] = np.nan #according to visualization part
...
for param in parameters:
    df[f"day_range_{param}"] = (df[f"d1_{param}_max"].subtract(df[
                                                                                               f"d1_{param}_min"]))  # .div(df[f"d1_{param}_max"])#range day normalizedthe subtraction is col-col
    df[f"hour_range_{param}"] = (df[f"h1_{param}_max"].subtract(
        df[f"h1_{param}_min"]))  # .div(df[f"h1_{param}_max"])#range hour normalized

    df[f"day_mean_{param}"] = df[[f"d1_{param}_max", f"d1_{param}_min"]].mean(
        axis=1)  # mean day
    df[f"hour_mean_{param}"] = df[[f"h1_{param}_max", f"h1_{param}_min"]].mean(
        axis=1)  # mean hour


#glucose all Mathematical operations
...
df['glucose_apache'] = df.apply(
    lambda x: largest_dis_from(tresh=130, d_max=x['d1_glucose_max'], d_min=x['d1_glucose_min'],
...
df['glucose_distance'] = df.apply(
    lambda x: distance_from(tresh=200, d_max=x['d1_glucose_max'], d_min=x['d1_glucose_min'],
                            h_max=x['h1_glucose_max'], h_min=x['h1_glucose_min']), axis=1)
